...
const MainPage = () => {
	...
	const [transferTxRecords, setTransferTxRecords] = useState<
		{ txHash: string; peanutLink: string }[]
	>([]);
	//logic for claiming
	...
	const [claimTxRecords, setClaimTxRecords] = useState<{ txHash: string }[]>(
		[]
	);
	...
	useEffect(() => {
		const timeOutId = setTimeout(async () => {
			const { railgunWalletInfo } = await getRailgunWallet(
				password,
				railgunWalletID,
				railgunWalletMnemonic
			);
			if (railgunWalletInfo) {
				...
				setWETHBalance(
					Number(
						await getPrivateBalance(railgunWalletInfo, TOKEN_ADDRESSES.WETH)
					)
				);
				setUSDCBalance(
					Number(
						await getPrivateBalance(railgunWalletInfo, TOKEN_ADDRESSES.USDC)
					)
				);
			}
		}, 300);
	...
	const onPrivateTransfer = async () => {
		...
		try {
			...
			const { railgunWalletInfo, encryptionKey } = await getRailgunWallet(
				password,
				railgunWalletID,
				railgunWalletMnemonic
			);
			...
			<Tabs variant="enclosed">
					...
					<TabPanel>
						<Box p={4} mb={4} borderRadius="md" boxShadow="md" pos="relative">
							<Box mb={4}>
								...
								<Input
									placeholder="0x123..."
									defaultValue={TOKEN_ADDRESSES.WETH}
							...
							<Button onClick={onPrivateTransfer} w="100%" mt="16px">
								Confirm
							</Button>
							{loading && (
								<Flex
                ...
                <Input placeholder="0zk123..." />
							...
							<Box mb={2}>
								...
								<Input
									placeholder="https://peanut..."
									onChange={(e) => setClaimPeanutLink(e.target.value)}
								/>
							</Box>
							{claimPeanutLink !== "" && (
								<Box>
							...
							<Button onClick={onPrivateClaim} w="100%" mt="16px">
								{receiveAsset === "USDC" ? "Withdraw & Swap" : "Withdraw"}{" "}
							</Button>
							{loading && (
								<Flex
									minH={200}
